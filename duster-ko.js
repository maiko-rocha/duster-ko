...
ko.dusterTemplateEngine.prototype = ko.utils.extend(new ko.templateEngine(), {
    // No deed for id at script tag
    // Can load complied template by template name
    // IE can put all compiled templates in one script file
    makeTemplateSource:function () {
        return new ko.templateSources.domElement($('<span></span>').get(0));
    },
    ...
    renderTemplateSource:function (templateSource, bindingContext, options) {
        ...
        for (var i in bindingContext.$parents) {
            // Fixed to bindingContext (was bindingContent ??)
            ctx = ctx.push(bindingContext.$parents[i]);
        }
